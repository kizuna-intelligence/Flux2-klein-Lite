...
本リポジトリ内のオリジナルソースコードは MIT ライセンスです
（Copyright 2025-2026 Fujitsu Ltd.）。

ただしこのランタイムは単独では動作せず、実行時に以下の第三者ソフトウェアに依存し、
それらを呼び出します。各々は自身のライセンスに従います:

- **diffusers**（Apache-2.0, Hugging Face）— 本ランタイムが読み込んで int4 化する
  `Flux2Transformer2DModel` のアーキテクチャ定義と config を提供。
- **GemLite**（Apache-2.0, MobiusML）— `gemlite` バックエンドが使う int4 GEMM カーネル本体。
  本リポジトリはパック形式の変換と呼び出しのみを行います。
- **PyTorch / Triton** — 実行時依存。

int4 のパック形式と dequant スキームは GPTQ / AutoGPTQ-v1 の規約に従います。
